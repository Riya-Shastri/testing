...
export class ReportingService {
  baseUrl = environment.dockerUrl;
  jwtHelperService = new JwtHelperService();
  decodedToken: any;

  constructor(private http: HttpClient) {}

  getSummary(): Observable<ResponseModel> {
    return this.http.get<ResponseModel>(
      this.baseUrl +
        "notification/getNotifications?clientDate=" +
  ...
  getNotificationList(requestForm: RequestModel): Observable<ResponseModel> {
    return this.http.post<ResponseModel>(
      this.baseUrl + "notification/getNotificationList",
      requestForm
    );
  }
  ...
  getNotificationsLite(): Observable<ResponseModel> {
    return this.http.get<ResponseModel>(
      this.baseUrl + "notification/getNotificationsLite",
      httpOptions
    );
  }
